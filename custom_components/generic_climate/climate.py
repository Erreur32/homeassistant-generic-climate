...
class GenericClimate(ClimateEntity, RestoreEntity):
    ...
    def __init__(
        ...
        if HVACMode.OFF in self._hvac_list and len(self._hvac_list) > 1:
            self._support_flags |= (
    ...
    async def async_added_to_hass(self):
        ...
        if self._cooler_entity_id:
            self.async_on_remove(
                async_track_state_change_event(
                    self.hass, [self._cooler_entity_id], self._async_switch_changed
                )
            )
    ...
    def hvac_modes(self):
        ...
        return [mode.value for mode in self._hvac_list]
    ...
    async def _async_control_heating_cooling(self, time=None, force=False):
        if self._hvac_mode == HVACMode.COOL:
            await self._async_control_cooling(time=time, force=force)
        elif self._hvac_mode == HVACMode.HEAT:
            await self._async_control_heating(time=time, force=force)
        else:
            if self._is_device_active:
                await self._async_heater_turn_off()
                await self._async_cooler_turn_off()
    ...
    async def _async_control_heating(self, time=None, force=False):
        ...
        async with self._temp_lock:
            ...
            if not self._active or self._hvac_mode != HVACMode.HEAT:
                return
    ...
    async def _async_control_cooling(self, time=None, force=False):
        ...
        async with self._temp_lock:
            ...
            if not self._active or self._hvac_mode != HVACMode.COOL:
                return
            ...
            if self._is_cooler_active:
                if too_cold:
                    ...
                    _LOGGER.info(
                        "Keep-alive - Turning on cooler %s", self._cooler_entity_id
                    )
    ...
    def _is_heater_active(self):
        """If the heater is currently active."""
        return self.hass.states.is_state(self.heater_entity_id, STATE_ON)

    @property
    def _is_cooler_active(self):
        """If the cooler is currently active."""
        return self.hass.states.is_state(self._cooler_entity_id, STATE_ON)
    ...
    async def async_turn_on(self):
        """Turn the entity on."""
        if self._hvac_mode == HVACMode.OFF:
            # Restore to previous mode or default to HEAT
            if HVACMode.HEAT in self._hvac_list:
                self._hvac_mode = HVACMode.HEAT
            elif HVACMode.COOL in self._hvac_list:
                self._hvac_mode = HVACMode.COOL
            else:
                self._hvac_mode = self._hvac_list[1]
            await self._async_control_heating_cooling(force=True)
            self.async_write_ha_state()
